...
do
  local function UpdateSplashes(_, Event, _, SourceGUID, _, _, _, DestGUID, _, _, _, SpellID)
    ...
    if Event ~= "SPELL_DAMAGE" and NucleusAbility.Type == "DirectDamage" then return end
...
-- Type: GroundDirectDamage         = Ability like Cataclysm where the area of effect deals direct damage one time.
--                                    We listen only on the SPELL_DAMAGE event.
--                                    The main target is considered to be the current target of the source if it has been hit.
...
function Splash.RegisterNucleusAbility(Type, SpellID, Radius)
  assert(type(Type) == "string" and (Type == "DirectDamage" or Type == "PeriodicDamage"), "Invalid or Unsupported Type.")
...
function Splash.RegisterNucleusAbilities()
  ...
  -- Death Knight
  -- Commons
  -- RegisterNucleusAbility("GroundMultipleDirectDamage", 43265, 8 * 2)   -- Death and Decay
  -- Blood
  ...
  -- Unholy
  -- RegisterNucleusAbility("GroundMultipleDirectDamage", 152280, 8 * 2)  -- Defile
